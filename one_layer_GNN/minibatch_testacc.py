...
def evaluate(model, g, nfeat, labels_original, test_nid,device):
    model.eval()
    with th.no_grad():
        pred = model.inference(g, nfeat, device)
    model.train()
    return compute_acc(pred[test_nid], labels_original[test_nid])
...
def run(args, device, data):
    ...
    for epoch in range(args.num_epochs):
        for step, (input_nodes, seeds, blocks) in enumerate(dataloader):
            ...
            loss.backward()
            optimizer.step()
            if iteration_num%args.log_every==0 or iteration_num==args.num_iterations-1:
                test_acc= evaluate(
                ...
                print(
                    "Epoch {:05d} | Step {:05d} | iter {:06d}| bt {:03d} | lr {:4f}| fo {:01d} |Loss {:.8f} |Test acc {:.4f}".format(
                        epoch,
                        step,
                        iteration_num,
                        args.batch_size,
            ...
            iteration_num+=1
            if iteration_num>=args.num_iterations:
